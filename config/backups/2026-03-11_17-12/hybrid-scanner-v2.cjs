...
const TOP_PAIRS = 50;  // HFT: was 250 � only top 50 liquid pairs
const ALL_PAIRS_MAX = 100;  // HFT: was 500 � only top 100 for 15m
...
function scan1m(symbol) {
  ...
  const longSignal = kCrossAboveD && inOversold;  // V6: puur StochRSI, geen EMA gate
  const shortSignal = kCrossBelowD && inOverbought && (price < emaTv) && k > 90;  // V9: shorts alleen bij extreme overbought >90 + onder EMA
  if (longSignal) {
...
async function main() {
  ...
  const usdtPairs = Object.values(markets)
    ...
    .filter(p => p.volume >= 10_000_000);  // HFT: min $10M 24h volume
