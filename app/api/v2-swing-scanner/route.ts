...
export async function GET() {
  ...
  try {
    const raw = await readFile(join(process.cwd(), 'public', 'v2-swing-signals.json'), 'utf-8');
    cachedData = raw;
    cacheTs = now;
    ...
    return NextResponse.json({ success: false, error: 'V2 swing scanner data not available' }, { status: 500 });
  }
}
